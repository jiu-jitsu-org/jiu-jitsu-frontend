...
export function FeedEmptyState() {
  ...
  return (
    ...
    <div className="flex min-h-screen flex-col items-center px-6 pt-[35vh] text-center">
      {/* 제목: Body M(16 Medium) */}
      <p className="text-base font-medium text-empty-state-default-title-text">
        아직 등록된 글이 없어요
      </p>
      {/* 부제목: Body S(14 Medium), 제목과 간격 4 */}
      <p className="mt-1 text-sm font-medium text-empty-state-default-description-text">
        가장 먼저 글을 작성해보세요!
      </p>
      ...
      <button
        type="button"
        onClick={openWrite}
        className="mt-[13px] h-[38px] rounded-[10px] bg-button-neutral-default-bg px-4 text-base font-semibold text-button-neutral-default-text"
      >
        글쓰기
      </button>
    </div>
