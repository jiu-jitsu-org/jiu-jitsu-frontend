 ...
 * 기존 메인 화면 백업.
 *
 * WHY: 인증 테스트 하니스를 메인(`/`)에 올리기 위해 원래 랜딩을 여기로 보관한다.
 * `_backup`은 App Router의 비공개 폴더(언더스코어 프리픽스)라 라우트가 생성되지 않는다.
 * 복구하려면 이 컴포넌트를 다시 `src/app/page.tsx`로 옮기면 된다.
 * (테스트 하니스 제거 시 이 파일도 함께 정리)
 */
