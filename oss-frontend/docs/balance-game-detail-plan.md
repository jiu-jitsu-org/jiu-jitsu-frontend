...
| 마감 | `투표가 종료되었어요`, 깜빡임 정지 | **양쪽 다 연한 배경** (내 선택 강조 없음) | 회색 텍스트 |
...
스크롤 위치는 **투표 영역의 높이가 변하지 않아야** 유지된다. 투표 후 상태에서 마감으로 갈 때 진행률 바가 이미 떠 있으므로 높이가 같지만, **미투표 상태에서 마감으로 갈 때는 % 가 새로 생겨 높이가 늘어난다.** → % 자리를 투표 전에도 확보해 두거나, 마감 전환 시 스크롤을 보정한다.
...
1. **마감 전환 시 레이아웃 점프** — 미투표 → 마감이면 % 가 새로 생긴다. Phase 6의 핵심 과제
