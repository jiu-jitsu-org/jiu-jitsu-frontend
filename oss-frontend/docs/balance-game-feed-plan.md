...
| 항목 | 결정 |
|---|---|
| 재투표 | 기획 원안은 **불가**. 다만 **취소(고른 것 재탭)는 테스트 편의로 열어 둔 상태** — 한 번 누르면 되돌릴 수 없어 확인이 어려웠다. 기획자에게 정책 수정 요청 예정이며, 원안 확정 시 `allowCancel: false`로 되돌린다. **변경(A→B)은 막힌 채**다 |
| 재투표 재허용 가능성 | 있음 → 판단을 **`balance-vote-policy.ts` 한 곳**에 모았다. `allowCancel` / `allowChange` 두 플래그로 조합이 갈리고, 투표 훅과 선택지 커서가 같은 출처를 읽는다 |
...
| 로그인 후 동작 | **투표는 자동 실행 안 함** → 다시 눌러야 함(pending action에 no-op). **상세 진입은 자동 이동**(기획: "로그인 완료 후 상세로 이동", 작성 FAB과 동일). 이동은 되돌릴 수 있지만 투표는 어렵다 |
